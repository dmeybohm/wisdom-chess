...
struct PieceInfo
{
    PieceInfo() : row { 0 }, column { 0 }, pieceImage { "" }, piece { wisdom::Piece_And_Color_None }
    {
    }

    PieceInfo (int row, int column, wisdom::ColoredPiece piece, QString pieceImage) :
            row { row }, column { column }, piece { piece }, pieceImage { std::move (pieceImage) }
    {
    }
...
public slots:
    void playerMoved (wisdom::Move selected_move, wisdom::Color who);
