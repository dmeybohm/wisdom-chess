...
namespace wisdom::analysis
{
    ...
    public:
        void commit_transaction ()
        {
            ...
            int result = sqlite3_exec (my_sqlite, "COMMIT", nullptr, nullptr, &errmsg);
        ...
        explicit SqliteHandle (const string& path, const Logger& logger) :
                my_sqlite { nullptr }, my_logger { &logger }
        {
        ...
        SqliteHandle (SqliteHandle&& other) noexcept : my_logger { other.my_logger }
        {
        ...
        void exec (const string& str) { exec (str.c_str ()); }
        ...
        void exec (czstring query) // NOLINT(readability-make-member-function-const)
        {
            ...
            wisdom::zstring errmsg = nullptr;
            sqlite3_exec (my_sqlite, query, nullptr, nullptr, &errmsg);
            if (errmsg != nullptr)
                do_abort (errmsg);
        }
    ...
    {
        db->exec ("CREATE TABLE IF NOT EXISTS iterative_searches ("
                  "    id INT PRIMARY KEY, "
                  "    color INT NOT NULL, "
                  "    fen varchar(50),  "
                  "    created DATETIME "
        ...
        db->exec ("CREATE TABLE IF NOT EXISTS positions ( "
                  ...
                  "   score int "
                  ")");
        db->exec ("CREATE TABLE IF NOT EXISTS transposition_hits ( "
                  "  id INT PRIMARY KEY, "
    ...
    public:
        SqlitePosition (shared_ptr<SqliteHandle> handle, const Board& board,
                        const SearchId& search_id,
                        const DecisionId& decision_id, Move move) :
        ...
        ~SqlitePosition () override
        {
            string insert = string ("INSERT INTO positions (id, decision_id, move, score) VALUES (")
                + my_position_id.to_string () + "," + my_decision_id.to_string () + "," + "'"
                + wisdom::to_string (my_move) + "'," + std::to_string (my_score) + ")";
    ...
    public:
        SqliteDecision (shared_ptr<SqliteHandle> handle, const Board& board,
                        const SearchId& search_id, const DecisionId& parent_id, int depth) :
                my_handle { std::move (handle) },
                my_board { &board },
                my_search_id { search_id },
                my_decision_id {},
        ...
        ~SqliteDecision () override
        {
            string parent_id_str
                = my_parent_id == Uuid::Nil () ? "NULL" : my_parent_id.to_string ();
            string move_str = my_move.has_value () ? "'" + to_string (*my_move) + "'" : "NULL";
            string parent_pos_id_str = my_parent_position_id == Uuid::Nil ()
                ? "NULL"
                : my_parent_position_id.to_string ();
            string query = "INSERT INTO decisions (id, search_id, parent_position_id, "
                           "parent_decision_id, depth, move) "
                           " VALUES ("
                + my_decision_id.to_string () + "," + my_search_id.to_string () + ","
                + parent_pos_id_str + ", " + parent_id_str + ", " + std::to_string (my_depth) + ","
                + move_str + ")";
    ...
    public:
        SqliteSearch (shared_ptr<SqliteHandle> handle, const Board& board, IterationId iteration_id,
                      int depth);

        ~SqliteSearch () override
        {
            my_handle->exec ("INSERT INTO searches (id, iteration_id, depth, created) VALUES ("
                             + my_search_id.to_string () + "," + my_iteration_id.to_string () + ","
                             + std::to_string (my_depth) + "," + "datetime('now'))");
        }
        ...
        {
            auto impl = std::make_unique<SqliteDecision> (my_handle, *my_board, my_search_id,
                                                          Uuid::Nil (), 0);
    ...
    public:
        SqliteIteration (shared_ptr<SqliteHandle> handle, const Board& board,
                         const IterativeSearchId& iterative_search_id, int depth) :
                my_handle { std::move (handle) },
                my_board { &board },
        ...
        ~SqliteIteration () override
        {
            my_handle->exec (
                ...
                + my_iteration_id.to_string () + "," + my_iterative_search_id.to_string () + ","
                + std::to_string (my_depth) + "," + "datetime('now'))");
        }
        ...
        {
            auto impl
                = std::make_unique<SqliteSearch> (my_handle, *my_board, my_iteration_id, my_depth);
    ...
    public:
        ...
        ~SqliteIterativeSearch () override
        {
            ...
            my_handle->exec ("INSERT INTO iterative_searches (id, fen, color, created) VALUES ("
                             + my_iterative_search_id.to_string () + "," + "'" + my_fen + "'" + ","
                             + std::to_string (index) + "," + "datetime('now'))");
        }
    ...
    public:
        explicit SqliteAnalytics (string file_path, const Logger& logger) :
                my_file_path { std::move (file_path) }, my_logger { &logger }
        {
        }
