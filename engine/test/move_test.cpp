...
TEST_CASE( "Pawn count" )
{
    ...
    SUBCASE( "Is updated from the amount of pawns on the board")
    {
        CHECK( brd->get_pawn_count (Color::Black) == 2 );
        CHECK( brd->get_pawn_count (Color::White) == 1 );
        CHECK( default_board.get_pawn_count (Color::White) == 8 );
        CHECK( default_board.get_pawn_count (Color::Black) == 8 );
    }

    SUBCASE( "Is unaffected by a pawn doing a capture" )
    {
    ...
    SUBCASE( "Is saved and restored after a pawn being captured" )
    {
        ...
        auto undo_state = brd->make_move (Color::White, capture_move);
        CHECK( brd->get_pawn_count (Color::Black) == 1 );
        CHECK( brd->get_pawn_count (Color::White) == 1 );
    ...
    {
        brd->set_current_turn (Color::Black);
        auto promoting_move = move_parse ("e2xd1 (Q)", Color::Black);
